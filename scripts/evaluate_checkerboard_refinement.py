...
from lcfm.metrics import mean_path_length, path_length_ratio, trajectory_acceleration, wasserstein_match
...
def evaluate_samples(model, x0, target, schedule, nfe, grid):
    if grid is None:
...
def plot_summary(out_path, profile, rows, samples_by_name, x0, target, nfe_for_points):
    ...
    err_norm = err / err.max()
    rho025 = (err + 1e-3).pow(0.25)
    rho05 = (err + 1e-3).pow(0.5)
    axes[0, 0].plot(ts, err_norm, color="#222222", lw=2.2, label="E1 profile")
    axes[0, 0].plot(ts, rho025 / rho025.max(), color="#2b7bba", ls="--", label="density p=0.25")
    axes[0, 0].plot(ts, rho05 / rho05.max(), color="#1b9e77", ls=":", label="density p=0.5")
    ...
    fig.suptitle("Checkerboard refinement schedule diagnostic", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate schedules on a trained 2D checkerboard-refinement CFM.")
    ...
    parser.add_argument("--plot-nfe", type=int, default=5)
    parser.add_argument("--out-dir")
    args = parser.parse_args()

    run_dir = Path(args.run_dir)
    config = read_json(run_dir / "config.json")
    if config["dataset"] != "checkerboard_refinement":
        raise SystemExit("This evaluator expects dataset='checkerboard_refinement'.")
    ...
    out_dir = Path(args.out_dir) if args.out_dir else run_dir / "eval_schedules"
    ...
    for nfe in parse_ints(args.nfe_values):
        ...
        for spec in specs:
            eval_result = evaluate_samples(model, x0, target, spec["schedule"], nfe, spec["grid"])
            ...
            rows.append(metrics)
            print({k: v for k, v in metrics.items() if k != "time_grid"}, flush=True)

    with (out_dir / "metrics.csv").open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=sorted({key for row in rows for key in row if key != "time_grid"}))
        writer.writeheader()
        for row in rows:
            writer.writerow({key: value for key, value in row.items() if key != "time_grid"})
    serializable = {
        "run_dir": str(run_dir),
    ...
    write_json(out_dir / "metrics.json", serializable)
    plot_summary(
